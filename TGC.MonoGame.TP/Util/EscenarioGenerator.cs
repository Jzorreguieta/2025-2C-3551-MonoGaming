...
internal class EscenarioGenerator
{
    ...
    public void GenerarEscenario( ref List<IModule> escenario, ContentManager contentManager, string contentFolder3D, string contentFolderEffects)
    {
        if (escenario is null)
        {
            //Inicializo el escenario  con 3 modulos vacios para dar tiempo de reaccion.
            ...
            for(int index = 3; index <= MAX_MODULES; index++)
            {
                ...
                lastPosition = index;
                escenario.Add(SeleccionarModulo(contentManager, contentFolder3D, contentFolderEffects, worldMatrix));
            }
        }
    }
    ...
    public IModule SeleccionarModulo(ContentManager contentManager, string contentFolder3D, string contentFolderEffects,Matrix worldMatrix)
    {
